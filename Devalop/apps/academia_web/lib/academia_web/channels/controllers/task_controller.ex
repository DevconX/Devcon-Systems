defmodule AcademiaWeb.TaskController do
  ...
  def create(conn, params) do
    ...
    case Workshop.add_topic_item(params) do
      {:ok, _} -> redirect(conn, to: "/topics/#{topic_id}")
      {:error, _} -> redirect(conn, to: "/topics/#{topic_id}")
    end
  end
  ...
  def delete(conn, %{"id" => task_id}) do
    case Workshop.delete_topic_item(task_id) do
      {:ok, topic_id, _} -> redirect(conn, to: "/topics/#{topic_id}")
      {:error, topic_id, _} -> redirect(conn, to: "/topics/#{topic_id}")
    end
  end
end